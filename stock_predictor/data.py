        ...
        if _can_use_twse_fallback(config):
            try:
                ...
                raise MarketDataError(
                    f"{config.symbol}: Yahoo 與 TWSE 備援都失敗。Yahoo: {yahoo_error}; TWSE: {twse_error}"
                ) from twse_error
        raise yahoo_error
...
def _fetch_yahoo_ohlcv(config: MarketConfig, years: int) -> pd.DataFrame:
    ...
    try:
        ...
        raise MarketDataError(f"{config.symbol}: 無法連線取得股價資料: {exc}") from exc
    except ValueError as exc:
        raise MarketDataError(f"{config.symbol}: 股價資料回應不是有效 JSON") from exc
    chart = payload.get("chart", {})
    error = chart.get("error")
    if error:
        description = error.get("description") or error.get("code") or "未知錯誤"
        raise MarketDataError(f"{config.symbol}: Yahoo Finance 回傳錯誤: {description}")
    result = (chart.get("result") or [None])[0]
    if not result or not result.get("timestamp"):
        raise MarketDataError(f"{config.symbol}: 找不到可用的歷史股價")
    ...
    if not quote_data:
        raise MarketDataError(f"{config.symbol}: 股價欄位缺失")
    frame = pd.DataFrame(
        {
    ...
    frame["Adj Close"] = adjclose_data.get("adjclose") if adjclose_data and adjclose_data.get("adjclose") is not None else frame["Close"]
    ...
    if len(frame) < 260:
        raise MarketDataError(f"{config.symbol}: 有效資料太少，至少需要約 1 年交易日")
    return frame
...
def _fetch_twse_ohlcv(config: MarketConfig, years: int) -> pd.DataFrame:
    ...
    for month in months:
        ...
        try:
            response = requests.get(url, params=params, timeout=20, headers=REQUEST_HEADERS, verify=False)
            response.raise_for_status()
            payload = response.json()
        except requests.RequestException as exc:
            raise MarketDataError(f"TWSE 無法連線: {exc}") from exc
        except ValueError as exc:
            raise MarketDataError("TWSE 回應不是有效 JSON") from exc
        if payload.get("stat") != "OK" or not payload.get("data"):
            continue
        frames.append(_parse_twse_month(payload["data"], stock_no=config.symbol))
    if not frames:
        raise MarketDataError("TWSE 沒有回傳可用歷史資料")
    ...
    if len(frame) < 260:
        raise MarketDataError("TWSE 有效資料太少，至少需要約 1 年交易日")
    return frame
...
def _parse_twse_month(rows: list[list[str]], stock_no: str) -> pd.DataFrame:
    ...
    for row in rows:
        try:
            ...
            continue
        records.append({"Date": date, "Open": open_, "High": high, "Low": low, "Close": close, "Adj Close": close, "Volume": volume, "Symbol": stock_no})
